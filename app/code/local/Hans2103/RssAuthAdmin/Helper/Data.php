...
class Hans2103_RssAuthAdmin_Helper_Data extends Mage_Rss_Helper_Data
{
    ...
    public function authAdmin($path)
    {
        ...
        if ($user && $user->getId() && $user->getIsActive() == '1'
            && $this->_adminSession->isAllowed($path)
        ) {
            ...
            if ($adminUserExtra && !is_array($adminUserExtra)) {
                ...
                try {
                    $adminUserExtra = Mage::helper('core/unserializeArray')
                        ->unserialize($user->getExtra());
                } catch (Exception $e) {
                    $adminUserExtra = [];
                }
            }
            if (!isset($adminUserExtra['indirect_login'])) {
                $adminUserExtra = array_merge(
                    $adminUserExtra, array('indirect_login' => true)
                );
